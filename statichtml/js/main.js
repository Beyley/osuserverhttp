...
$(document).ready(function(){$("#pagecontent img").hover(function(){if($(this).width()>=660)
$(this).addClass("expandable");},function(){$(this).removeClass("expandable");});$("#pagecontent img").click(function(){$(this).toggleClass("expanded");});$(".form-text").formDefaults();$('#quickreply-text').keydown(function(e){if(e.ctrlKey&&e.keyCode==13){$("#quickreplybutton").click();return false;}});handleFrozenHeader();handleClickableCells();});function handleFrozenHeader()
{var fixable=$("#fixable");if(!fixable.length)return;var clone=null;$(window).resize(function(){if(clone!==null)clone.css("left",(fixable.offset().left- $(window).scrollLeft())+'px');});$(window).scroll(function(){if(clone!==null)clone.css("left",(fixable.offset().left- $(window).scrollLeft())+'px');if($(window).scrollTop()>=fixable.offset().top)
{if(clone===null)
{clone=fixable.clone();clone.addClass(fixable.is(".minimal")?"frozen-header minimal":"frozen-header").css("width",fixable.width()+'px').css("left",fixable.offset().left+'px');$('body').append(clone);}}
else
{if(clone!==null)
...
{location.href='https://web.archive.org/web/20120517155233/http://www.google.com/translate?u='+encodeURIComponent(location.href)+'&langpair=en%7C'+langCode+'&hl=en&ie=UTF8';return false;}
...
{area.html("Processing...");area.load("/web/favourite.php?a="+id);return false;}
function updateChat()
{$("#chat").load("/pages/include/home-ircfeed.php",{limit:25},function(){setTimeout(updateChat,10000);});}
function updateRecent()
{$("#recent_plays").load("/pages/include/home-recentplays.php",{limit:25});setTimeout('updateRecent()',30000);}
function updateMostPlayed()
{$("#most_played").load("/pages/include/home-mostplayed.php",{limit:25},function(){setTimeout(updateMostPlayed,600000);});}
...
this.toggleClass(sf.c.shadowClass+'-off');};sf.c={bcClass:'sf-breadcrumb',menuClass:'sf-js-enabled',anchorClass:'sf-with-ul',arrowClass:'sf-sub-indicator',shadowClass:'sf-shadow'};sf.defaults={hoverClass:'sfHover',pathClass:'overideThisToUse',pathLevels:1,delay:400,animation:{opacity:'show'},speed:80,autoArrows:true,dropShadows:true,disableHI:false,onInit:function(){},onBeforeShow:function(){},onShow:function(){},onHide:function(){}};$.fn.extend({hideSuperfishUl:function(){var o=sf.op,not=(o.retainPath===true)?o.$path:'';o.retainPath=false;var $ul=$(['li.',o.hoverClass].join(''),this).add(this).not(not).removeClass(o.hoverClass).find('>ul').hide().css('visibility','hidden');o.onHide.call($ul);return this;},showSuperfishUl:function(){var o=sf.op,sh=sf.c.shadowClass+'-off',$ul=this.addClass(o.hoverClass).find('>ul:hidden').css('visibility','visible');sf.IE7fix.call($ul);o.onBeforeShow.call($ul);$ul.animate(o.animation,o.speed,function(){sf.IE7fix.call($ul);o.onShow.call($ul);});return this;}});})(jQuery);(function($){$.fn.formDefaults=function(options){var opts=$.extend({},$.fn.formDefaults.defaults,options);return this.each(function(){var $this=$(this);var $form=$this.parents("form");$this.data("defaultValue",this.value).addClass("form-default-value-processed");if(opts.inactiveColor){$this.css("color",opts.inactiveColor);}
$this.focus(function(){if(this.value==$this.data("defaultValue")){this.value='';this.style.color=opts.activeColor;}}).blur(function(){if(this.value===''){this.style.color=opts.inactiveColor;this.value=$this.data("defaultValue");}});if(!$form.data("defaultValueProcessed")){$form.data("defaultValueProcessed",true).submit(function(e){$(this).find(".form-default-value-processed").each(function(){var $el=$(this);if($el.data("defaultValue")==$el.val()){$el.val('');}});});}});};$.fn.formDefaults.defaults={activeColor:'#000',inactiveColor:'#b9bab5'};}(jQuery));function popup(url,width,height)
...
{$(root).parents(".spoiler").children(".spoiler_body").slideToggle("fast");return false;}
function expandPost()
{$("#truncated").hide(0);$("#full").show(0);return false;}
function userSearch()
{document.location="/u/"+ $("#user-search").val();return false;}
function beatmapSearch()
{document.location="/p/beatmaplist?q="+ $("#beatmap-search").val();return false;}

}
/*
     FILE ARCHIVED ON 15:52:33 May 17, 2012 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 23:39:48 Dec 22, 2020.
...
playback timings (ms):
  ...
  esindex: 0.01
  LoadShardBlock: 423.772 (3)
  PetaboxLoader3.datanode: 201.27 (4)
  CDXLines.iter: 20.96 (3)
  load_resource: 78.948
  PetaboxLoader3.resolve: 36.177
*/